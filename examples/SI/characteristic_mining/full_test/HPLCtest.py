...
def extract_HPLC(input_text, api_key, model="gpt-4", base_url="https://api.openai.com/v1/chat/completions"):
    system_prompt = f"""
    You're an expert in chemistry. You will be provided with a text may containing HPLC data and lots of other information. I need you to find and summarize HPLC data in the text and output it in markdown tabular form.
    I need you to find the code name of the molecule, and find column type, solvent, flow rate, wavelength, major retention time and minor retention time that correspond to that code, and organize them in a single row.
    ...
    You should only output the table containing 7 columns: | code name | column type | solvent | flow rate | wavelength | minor retention time | major retention time |
...
Tetramethylsilane (TMS) served as the internal standard for 1H NMR, and CDCl and CD CNserved as the internal standard for 13C NMR. The following abbreviations were used to express the<|endoftext|>'''>
        Answer:
        | code name | column type | solvent | flow rate | wavelength | minor retention time | major retention time |
        |-----------|-------------|---------|-----------|------------|----------------------|----------------------|
...
28.84, 27.44, 25.76. IR (KBr, cm-1): 2938, 2861, 1707, 1509, 1231, 812, 752; HRMS (ESI) calcd<|endoftext|>'''>
        ...
        | code name | column type | solvent | flow rate | wavelength | minor retention time | major retention time |
        |-----------|-------------|---------|-----------|------------|----------------------|----------------------|
        | 3aa | Daicel Chiralpak AS-H | hexane/iso-propanol = 95:5 | 0.7 mL/min | 251 nm | 9.60 min | 11.11 min |
        | 3ad | Daicel Chiralpak AS-H | hexane/iso-propanol = 95:5 | 0.7 mL/min | 251 nm | 10.29 min | 12.19 min |
...
CDCl ) δ 211.87, 148.05, 140.18, 134.45, 129.08, 127.96,<|endoftext|>'''>
        ...
        | code name | column type | solvent | flow rate | wavelength | minor retention time | major retention time |
        |-----------|-------------|---------|-----------|------------|----------------------|----------------------|
        | 3ab | Daicel Chiralpak AS-H | hexane/iso-propanol = 95:5 | 0.7 mL/min | 251 nm | 10.76 min | 12.75 min |
...
------
    ...
    if test_text == '\n':#新加的
        print('''| code name | column type | solvent | flow rate | wavelength | minor retention time | major retention time |
        |-----------|-------------|---------|-----------|------------|----------------------|----------------------|''')
        print('\n')
        return '''| code name | column type | solvent | flow rate | wavelength | minor retention time | major retention time |
        |-----------|-------------|---------|-----------|------------|----------------------|----------------------|'''
