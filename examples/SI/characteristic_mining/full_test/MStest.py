...
def extract_MS(input_text, api_key, model="gpt-4", base_url="https://api.openai.com/v1/chat/completions"):
    system_prompt = f"""
        You're an expert in chemistry. You will be provided with a text may containing MS data and lots of other information. I need you to find and summarize MS data in the text and output it in markdown tabular form.
        I need you to find the code name of the molecule, and find the name of MS, method of MS, calculated mass and experimental mass, and organize them in a single row.
        ...
        You should only output the table containing 5 columns: | code name | name of MS | method of MS | calculated mass | experimental mass |
...
Tetramethylsilane (TMS) served as the internal standard for 1H NMR, and CDCl and CD CNserved as the internal standard for 13C NMR. The following abbreviations were used to express the<|endoftext|>'''>
        Answer:
        | code name | name of MS | method of MS | calculated mass | experimental mass |
        |-----------|------------|--------------|-----------------|-------------------|
...
28.84, 27.44, 25.76. IR (KBr, cm-1): 2938, 2861, 1707, 1509, 1231, 812, 752; HRMS (ESI) calcd<|endoftext|>'''>
        ...
        | code name | name of MS | method of MS | calculated mass | experimental mass |
        |-----------|------------|--------------|-----------------|-------------------|
        | 3aa | HRMS | ESI | 306.1852 | 306.1848 |
...
1495, 1392, 1124, 807, 749; HRMS (ESI) calcd for C H ONCl+: 340.1458, found 340.1460.3ac:<|endoftext|>'''>
        Answer:
        | code name | name of MS | method of MS | calculated mass | experimental mass |
        |-----------|------------|--------------|-----------------|-------------------|
        | 3ab | HRMS | ESI | 340.1458 | 340.1460 |
...
------
    ...
    if test_text == '\n':#新加的
        print('''| code name | name of MS | method of MS | calculated mass | experimental mass |
        |-----------|------------|--------------|-----------------|-------------------|''')
        print('\n')
        return '''| code name | name of MS | method of MS | calculated mass | experimental mass |
        |-----------|------------|--------------|-----------------|-------------------|'''
